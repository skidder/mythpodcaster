...
public class UserDefinedTranscoderImpl extends AbstractTranscoderImpl implements Transcoder {
  ...
  public void transcode(File workingDirectory, GenericTranscoderConfigurationItem genericConfig,
    ...
    UserDefinedTranscoderConfigurationItem config =
        (UserDefinedTranscoderConfigurationItem) genericConfig;
    ...
    for (String argument : config.getParsedEncoderArguments()) {
      if (argument.contains(config.getInputVariableName())) {
        final String inputFileSub =
            argument.replace(config.getInputVariableName(), inputFile.getAbsolutePath());
        ...
        final String outputFileSub =
            argument.replace(config.getOutputVariableName(), outputFile.getAbsolutePath());
    ...
    try {
      ...
      throw new Exception("UserDefinedCommand process interrupted by another thread", e);
    } catch (ExecutionException ee) {
      throw new Exception("Something went wrong parsing UserDefinedCommand output", ee);
      ...
      throw new Exception("Something went wrong parsing UserDefinedCommand output", re);
    } finally {
      if (process != null) {
        process.destroy();
      }
