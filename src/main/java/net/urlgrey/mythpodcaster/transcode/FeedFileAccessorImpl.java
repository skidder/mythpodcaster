...
public class FeedFileAccessorImpl implements FeedFileAccessor {
   ...
   * @throws IOException
   */
  public SyndFeed readFeed(String seriesId, String transcodingProfileId, String title)
      throws IOException {
  ...
  public void addProgramToFeed(RecordedSeries series, RecordedProgram program, Channel channel,
      SyndFeed feed, String transcodingProfileId) {
    LOGGER.info("Transcoding new feed entry: programId[" + program.getProgramId() + "], key["
        + program.getKey() + "], title[" + program.getTitle() + "], channel["
        + (channel != null ? channel.getName() : "") + "], transcodingProfileId["
    ...
    final File originalClipThumbnail =
        clipLocator.locateThumbnailForOriginalClip(program.getFilename());
    if (originalClipThumbnail != null) {
      ...
      feedImageUrl =
          this.applicationURL + PATH_SEPARATOR + transcodingProfileId + PATH_SEPARATOR + seriesId
              + PNG_EXTENSION;
      ...
      try {
        ...
        LOGGER.info("Applied clip thumbnail to feed: thumbnail["
            + feedThumbnailFile.getAbsolutePath() + "], url[" + feedImage.getUrl() + "]");
      } catch (IOException e) {
    ...
    if (originalClip != null) {
      ...
      if (profile != null) {
        ...
        try {
          ...
          transcodingController.transcode(profile, program.getKey(), series.getTitle(),
              program.getProgramTitle(), originalClip, outputFile);
          ...
          if (outputFile.canRead()) {
            ...
            final Duration duration =
                new Duration(program.getEndTime().getTime()
                    - program.getRecordedProgramKey().getStartTime().getTime());
            ...
            LOGGER.warn("Transcoded output file cannot be read, setting link to null: path["
        ...
        final String msg =
            "Unable to locate transcoding profile with given id: [" + transcodingProfileId + "]";
        LOGGER.error(msg);
      }
    } else {
  ...
  public File generateTransformationFromFeed(File feedFile, SyndFeed feed, String seriesId)
    ...
    try {
      ...
      transformedOutputFile =
          new File(feedFile.getParentFile(), seriesId + feedTransformationOutputFileExtension);
      try {
        ...
        transformer.transform(new StreamSource(new FileInputStream(feedFile)), new StreamResult(
            fileOutputStream));
  ...
  @Required
  public void setFeedTransformationOutputFileExtension(String feedTransformationOutputFileExtension) {
    this.feedTransformationOutputFileExtension = feedTransformationOutputFileExtension;
  }

  @Required
  public void setFeedTransformationTemplateFile(File feedTransformationTemplateFile) {
