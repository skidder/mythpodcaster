...
public class GlobalTranscodeAndCleanupTaskImpl implements ApplicationContextAware {
  ...
  private void doWork() {
    ...
    for (FeedSubscriptionItem subscription : subscriptions) {
      ...
      try {
        feed =
            feedFileAccessor.readFeed(subscription.getSeriesId(),
                subscription.getTranscodeProfile(), subscription.getTitle());
      } catch (IOException e) {
        LOGGER.error("Continuing, error reading feed for recordId[" + subscription.getSeriesId()
            + "]", e);
        continue;
      }
      ...
      if (subscription.isActive() == false) {
        ...
        // subscription entry
        feedFileAccessor.purgeFeed(subscription.getSeriesId(), subscription.getTranscodeProfile(),
            feed);
    ...
    for (FeedSubscriptionItem subscription : subscriptions) {
      ...
      Runnable task =
          (Runnable) this.applicationContext.getBean("feedTranscodingTask", new Object[] {
              subscription, feed});
