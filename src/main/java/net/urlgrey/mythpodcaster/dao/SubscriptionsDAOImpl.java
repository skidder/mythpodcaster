...
public class SubscriptionsDAOImpl extends AbstractFileBasedDAO implements SubscriptionsDAO {
  ...
  public SubscriptionsDAOImpl() {
    try {
      jaxbContext =
          JAXBContext
              .newInstance(new Class[] {FeedSubscriptionItem.class, FeedSubscriptions.class});
  ...
  public synchronized void addSubscription(FeedSubscriptionItem item) throws IOException {
    ...
    LOGGER.debug("Adding subscription: seriesId [" + seriesId + "], transcodeProfileId["
    ...
    if (feedFile.exists() == false) {
      ...
      SyndFeed feed =
          feedFileAccessor.createFeed(feedFile, seriesId, title, item.getTranscodeProfile());
      if (feed == null) {
  ...
  public void removeSubscription(String seriesId, String transcodeProfileId) {
    LOGGER.debug("Removing subscription: seriesId [" + seriesId + "], transcodeProfileId["
        + transcodeProfileId + "]");
