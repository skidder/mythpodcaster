...
public class TranscodingProfileSubscriptionsPanel extends RemoteComposite {
  ...
  public void refreshData() {
    ...
    AsyncCallback<List<FeedSubscriptionItemDTO>> callback =
        new AsyncCallback<List<FeedSubscriptionItemDTO>>() {
          ...
          public void onSuccess(List<FeedSubscriptionItemDTO> subscriptions) {
            ...
            for (FeedSubscriptionItemDTO item : subscriptions) {
              ...
              subscriptionsTable.setWidget(i, 1, new HTML("<a target=_blank href=\""
                  + applicationUrl + "/" + item.getTranscodeProfile() + "/" + seriesId
                  + ".rss\">Feed</a>"));
              subscriptionsTable.setWidget(i, 2, new HTML("<a target=_blank href=\""
                  + applicationUrl + "/" + item.getTranscodeProfile() + "/" + seriesId
                  + ".html\">HTML</a>"));
              ...
              if ((item.getTranscodeProfileDisplayName() != null)
                  && (false == item.getTranscodeProfileDisplayName().startsWith(
                      UIControllerService.UNRECOGNIZED_PROFILE_LABEL))) {
                final Button editButton = new Button("Edit");
                editButton.addClickHandler(new AddSubscriptionHandler(parent, item
                    .getTranscodeProfile(), seriesId, seriesTitle));
                subscriptionsTable.setWidget(i, 3, editButton);
              }

              final Button unsubscribeButton = new Button("Unsubscribe");
              unsubscribeButton.addClickHandler(new UnsubscribeHandler(parent, seriesId, item
                  .getTranscodeProfile()));
